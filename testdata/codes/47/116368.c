...
void find_right(Tree *head , Tree *new_head , int *find) ;
void find_left(Tree *head , Tree *new_head , int *find) ;
...
int main()
{
    ...
    while(fgets(read_command , 100 , stdin) != NULL){
        if(read_command[0] == 'p'){
            ...
            int find_or_not = 0 ;
            find_left(head , new_head , &find_or_not) ;
        }else if(read_command[7] == 'r'){
            Tree *new_head ;
            new_head = (Tree *)malloc(sizeof(Tree)) ;
            ...
            int find_or_not = 0 ;
            find_right(head , new_head , &find_or_not) ;
        }else{
...
void find_right(Tree *head , Tree *new_head , int *find){
    if(*find || head == NULL)
        return ;
    if(head->right == NULL && head->middle == NULL && head->left == NULL){
        if(head->c == new_head->c){
            head->time += new_head->time ;
        }else{
            head->left = (Tree *)malloc(sizeof(Tree)) ;
    ...
    find_right(head->right , new_head , find) ;
    find_right(head->middle , new_head , find) ;
    find_right(head->left , new_head , find) ;
    if(*find)
        head->time += new_head->time ;
}
...
void find_left(Tree *head , Tree *new_head , int *find){
    ...
    find_left(head->left , new_head , find) ;
    find_left(head->middle , new_head , find) ;
    find_left(head->right , new_head , find) ;
    if(*find)
        head->time += new_head->time ;
}
