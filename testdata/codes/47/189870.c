...
Treenode* insertNode(Treenode *root, int pos, char x, int numOfchar){
    ...
    else if(pos>root->num+root->leftSubNum){
        root->rightSubNum += numOfchar;
        root->right=insertNode(root->right,pos-root->num-root->leftSubNum,x,numOfchar);
        return root;
    }
    else if(pos==root->num+root->leftSubNum){
        if(root->c==x){
